...
const BookingPage = () => {
  ...
  const bookingFunc = async () => {
    ...
    let roomData = {
      ...
      stay: state.bookingData.stay,
      guest: state.bookingData.guest,
      totalNights: state.bookingData.totalNights,
      totalPrice: (
        (state.bookingData.totalPrice / 100) * taxRate +
    ...
    try {
      let { data } = await axios.post("/booking/book", roomData);
      // console.log(data);
      ...
      // console.log(state.bookingData.length);
      // state.bookingData.map(async (item) => {
      //   console.log(item);
  ...
  const taxRate = state.bookingData.totalPrice < 2000 ? 0 : 12;
        ...
        <>
                ...
                {onlinePayment === "card" ? (
                  <div className="method">
                    <StripeCheckout
                      ...
                      amount={state.bookingData.totalPrice * 100} // paisa or cents
                      currency="USD"
                      // zipCode={false}
                      shippingAddress
                      billingAddress
                        ...
                        createOrder={(data, actions) => {
                          ...
                          return actions.order.create({
                            purchase_units: [
                              {
                                amount: {
                                  value: `${state.bookingData.totalPrice}.00`,
                                },
                              },
                            ],
                          });
          ...
          <Summary>
            ...
            <div className="order-summery">
              {state.bookingData.room.map((room) => (
                  ...
                  <div className="detail">
                    ...
                    <div className="down">
                      <div className="guest">
                        ...
                        <div>
                          Adult: {state.bookingData.guest.adult}, Children:{" "}
                          {state.bookingData.guest.children}
                        </div>
                      </div>
                      <div className="stay">
                        <div style={{ fontWeight: "bold" }}>Stay: </div>
                        <div>
                          {state.bookingData.stay.checkIn}-
                          {state.bookingData.stay.checkOut}
                        </div>
                      </div>
                      <div className="stay">
                        <div style={{ fontWeight: "bold" }}>Total Nights:</div>
                        <div>{state.bookingData.totalNights}</div>
                      </div>
                      <div className="price">
                        <div> Room Price:</div>
                        <div> Rs.{room.price}/Night</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="total">
                <div>
                  <div>Rooms Count:</div>
                  <div>{state.bookingData.room.length}</div>
                </div>

                <div>
                  <div>Room Total:</div>
                  <div>Rs. {state.bookingData.totalPrice}/Night</div>
                </div>
                ...
                >
                  <div>Tax Charges:</div>
                  <div>Rs. {state.bookingData.totalPrice/100 * taxRate}</div>
                  {/* <div>{state.bookingData.totalPrice}</div> */}
                </div>
                ...
                >
                  ...
                  <div style={{ color: "#000", fontWeight: "bold" }}>
                    Rs. {((state.bookingData.totalPrice/100 * taxRate) + state.bookingData.totalPrice).toFixed(2)}
                  </div>
                </div>
              </div>
            </div>
